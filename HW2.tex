...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{trapezoid.middle} \hlkwb{<-} \hldef{left.points[}\hlnum{2}\hlopt{:}\hlnum{100}\hldef{]}

\hldef{(total} \hlkwb{<-} \hldef{(}\hlkwd{sum}\hldef{(}\hlnum{2}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(trapezoid.middle)))} \hlopt{+} \hldef{(}\hlkwd{integrand}\hldef{(left.points[}\hlnum{1}\hldef{]))} \hlopt{+} \hldef{(}\hlkwd{integrand}\hldef{(right.points[}\hlnum{100}\hldef{])))}
  \hlopt{*}\hldef{delta.x} \hlopt{/}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hldef{\}}
  \hlkwa{if}\hldef{(a} \hlopt{>} \hldef{b)\{}
    \hlkwd{stop}\hldef{(}\hlsng{"bounds invalid"}\hldef{)}
  \hldef{\}}
  \hlcom{######################################}
  \hlcom{# Compute Area}
  \hlcom{######################################}
  \hldef{delta.x} \hlkwb{<-} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect}
  \hldef{left.sides} \hlkwb{<-} \hldef{a} \hlopt{+} \hlnum{0}\hlopt{:}\hldef{((n.rect}\hlopt{-}\hlnum{1}\hldef{))}\hlopt{*}\hldef{(delta.x)}
  \hldef{right.sides} \hlkwb{<-} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hldef{((n.rect))}\hlopt{*}\hldef{(delta.x)}

  \hlkwa{if}\hldef{(method} \hlopt{==} \hlsng{"Left"}\hldef{)\{}
    \hldef{area} \hlkwb{<-} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(left.sides)))}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Right"}\hldef{)\{}
    \hldef{area} \hlkwb{<-} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(right.sides)))}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Midpoint"}\hldef{)\{}
    \hldef{area} \hlkwb{<-} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{((left.points}\hlopt{+}\hldef{right.points)}\hlopt{/}\hlnum{2}\hldef{)))}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    \hldef{trapezoid.middle} \hlkwb{<-} \hldef{left.sides[}\hlnum{2}\hlopt{:}\hlnum{100}\hldef{]}
    \hldef{area} \hlkwb{<-} \hldef{(}\hlkwd{sum}\hldef{(}\hlnum{2}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(trapezoid.middle)))} \hlopt{+} \hldef{(}\hlkwd{integrand}\hldef{(left.sides[}\hlnum{1}\hldef{]))} \hlopt{+} \hldef{(}\hlkwd{integrand}\hldef{(right.sides[}\hlnum{100}\hldef{])))}\hlopt{*}\hldef{delta.x} \hlopt{/}\hlnum{2}
