...
\begin{enumerate}
  ...
  \textbf{Solution:}
<<size="scriptsize", warning=FALSE, message=FALSE>>=
trapezoid.middle <- left.points[2:100]

(total <- (sum(2*(integrand(trapezoid.middle))) + (integrand(left.points[1])) + (integrand(right.points[100])))
  *delta.x /2)
...
riemann.sums <- function(fnct,                        # function to integrate
  ...
  ######################################
  delta.x <- (b-a)/n.rect
  left.sides <- a + 0:((n.rect-1))*(delta.x)
  right.sides <- a + 1:((n.rect))*(delta.x)

  if(method == "Left"){
    area <- sum(delta.x*(integrand(left.sides)))
  }else if(method == "Right"){
    area <- sum(delta.x*(integrand(right.sides)))
  }else if(method == "Midpoint"){
    area <- sum(delta.x*(integrand((left.points+right.points)/2)))
  }else if(method == "Trapezoidial"){
    trapezoid.middle <- left.sides[2:100]
    area <- (sum(2*(integrand(trapezoid.middle))) + (integrand(left.sides[1])) + (integrand(right.sides[100])))*delta.x /2
  }else{
